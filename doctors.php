...
<!----------------------------------------------------------------------------------->
<?php
[$result, $error] = $db->query("SELECT doctors.id, doctors.name, doctors.email, doctors.phone, doctors.address FROM doctors");
                ...
                <div class="modal-body" id="modal-body">
                    <form id="edit-form">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="name" name="photo_name" required value="">
                        </div>
                        <div class="mb-3">
                            <label for="email" class="form-label">Email</label>
                            <input type="email" class="form-control" id="email" name="photo_email" required value="">
                        </div>
                        <div class="mb-3">
                            <label for="phone" class="form-label">Phone</label>
                            <input type="tel" class="form-control" id="phone" name="doctor_phone" required value="">
                        </div>
                        <div class="mb-3">
                            <label for="address" class="form-label">Address</label>
                            <input type="text" class="form-control" id="address" name="doctor_address" value="">
                        </div>
                        <input type="hidden" id="id" name="doctor_id">
                    </form>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-primary" data-bs-dismiss="modal" onclick="submitForm()">
                        Save
                    </button>
                </div>
            </div>
...
<script defer>
    ...
    function submitForm() {
        ...
        fetch('/api_doctor_edit.php', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded'
            },
            body: `doctor_name=${name}&doctor_email=${email}&doctor_phone=${phone}&doctor_address=${address}&doctor_id=${id}`
        }).then(response => {
            return response.json().catch(() => {
                alert('An error occurred while processing your request');
            })
    ...
    function setUpTable() {
        ...
        editButtons.forEach(button => {
            button.addEventListener('click', (event) => {
                const doctorId = button.id.split('_')[1];
                fetch(`/api_doctor_get.php?doctor_id=${doctorId}`)
                    .then(response => {
                        return response.json().catch(() => {
                            alert('An error occurred while processing your request');
                        })
                    ...
                    data => {
                        ...
                        form.querySelector('#id').value = doctorId;
        ...
        deleteButtons.forEach(button => {
            button.addEventListener('click', (event) => {
                const doctorId = button.id.split('_')[1];
                fetch(`/api_doctor_delete.php?doctor_id=${doctorId}`, {
                    method: 'DELETE'
                }).then(response => {
                    return response.json().catch(() => {
                        alert('An error occurred while processing your request');
