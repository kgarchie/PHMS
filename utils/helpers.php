...
{
    global $errors, $db;
    [$result, $error] = $db->query("SELECT * FROM users WHERE `email` = ? AND `password` = ?", $email, $password);
    if ($error) {
    ...
    $user_id = $user->get('id');
    $token = uniqid();
    [$result, $error] = $db->query("INSERT INTO tokens (`user_id`, `token`) VALUES (?, ?)", $user_id, $token);
    if ($error) {
        array_push($errors, $error);
        return null;
    }
